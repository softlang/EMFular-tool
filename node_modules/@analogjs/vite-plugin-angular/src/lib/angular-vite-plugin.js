    ...
    function getTsConfigPath(root, options, isProd, isTest, isLib) {
        if (options.tsconfig && isAbsolute(options.tsconfig)) {
            if (!existsSync(options.tsconfig)) {
                console.error(`[@analogjs/vite-plugin-angular]: Unable to resolve tsconfig at ${options.tsconfig}. This causes compilation issues. Check the path or set the "tsconfig" property with an absolute path.`);
            }
